...
GLfloat wallVertices[] = {
    // triangle 1
    -40.0, 0.0, -40.0, 40.0, 0.0, -40.0, 40.0, 30.0, -40.0,
    // triangle 2
    40.0, 30.0, -40.0, -40.0, 30.0, -40.0, -40.0, 0.0, -40.0};
...
enum Buffer_IDS {
  Position,
  UV,
  NumBuffers // calculated at compile-time, someone should write a book about
             // that
};

static GLuint Buffers[NumBuffers];

static GLuint wallsProgramID;

static GLuint mvpMatrixLoc;

static GLuint textureID;

static GLuint wallTextureLoc;
...
void main_scene_init_scene() {
  ...
  glGenBuffers(NumBuffers, Buffers);
  {
    ...
    // populate color buffer
    {
      ...
      GL_DEBUG_ASSERT();
      glBufferData(GL_ARRAY_BUFFER, sizeof(wallUVs), wallUVs, GL_STATIC_DRAW);
      GL_DEBUG_ASSERT();
    }

    enum Attribute_IDS { vPosition = 0, vUV = 1 };
  ...
  // load shaders
  {
    ...
    GLuint vertexShaderID =
        compile_shader(GL_VERTEX_SHADER, SHADER_DIR "walls.vert");

    GLuint fragmentShaderID =
        compile_shader(GL_FRAGMENT_SHADER, SHADER_DIR "walls.frag");

    wallsProgramID = link_shaders(vertexShaderID, fragmentShaderID);

    mvpMatrixLoc = glGetUniformLocation(wallsProgramID, "mvpMatrix");
    wallTextureLoc = glGetUniformLocation(wallsProgramID, "wallTexture");
  ...
  // initialize projection matrix
  {
    ...
    mat4_perspective(45.0f, (GLfloat)w / (GLfloat)h, 0.1f, 1000.0f);
  }
  ...
  // load textures
  {
    ...
    unsigned char *thegrid =
        stbi_load(TEXTURE_DIR "thegrid.png", &width, &height,
...
struct camera camera = {
    .x = 0.0, .y = 5.0, .z = 0.0, .rotationX = 0.0, .rotationY = 0.0};
...
void main_scene_draw_scene() {
  ...
  // translate from object coordinates to camera coordinates
  {
    ...
    mat4_rotateX(VIEW, camera.rotationX);
    mat4_rotateY(VIEW, -camera.rotationY);
    mat4_translate(VIEW, neg_camera);
  }
  ...
  // draw the four walls
  {
    ...
    for (uint8_t i = 0; i < 4; rotation += pi_over_two, i++) {

      mat4_rotateY(MODEL, rotation);
...
void main_scene_window_size_callback(GLFWwindow *window, int width,
                                     int height) {
  enum matrixType m = PROJECTION;
  mat4_identity(m);
  int32_t w, h;

  mat4_perspective(45.0f, (GLfloat)width / (GLfloat)height, 0.1f, 1000.0f);
}
